...
def test_fused_equals_manual_when_dropout_zero():
    ...
    B, T, E, H = 2, 12, 48, 6
    attn_fused = CausalSelfAttention(E, H, T, dropout_p=0.0, use_fused=True)
    attn_manual = CausalSelfAttention(E, H, T, dropout_p=0.0, use_fused=False)
...
def test_causality_no_leakage_into_future():
    torch.manual_seed(0)
    B, T, E, H = 1, 10, 32, 4
    attn = CausalSelfAttention(E, H, T, dropout_p=0.0, use_fused=True).eval()
...
def test_backward_and_grad_flow():
    ...
    attn = CausalSelfAttention(E, H, T, dropout_p=0.0, use_fused=True)
    y = attn(x)
    loss = y.pow(2).mean()
    loss.backward()
...
def test_get_causal_mask_raises_when_fused():
    E, H, T = 16, 4, 8
    attn_fused = CausalSelfAttention(E, H, T, use_fused=True)
...
def test_causal_mask_overflow_raises():
    E, H, T = 16, 4, 8
    attn_manual = CausalSelfAttention(E, H, T, use_fused=False)
    with pytest.raises(AssertionError):
...
def test_dropout_behavior_training_eval(use_fused):
    torch.manual_seed(0)
    E, H, T, B = 32, 4, 12, 2
    attn = CausalSelfAttention(E, H, T, dropout_p=0.5, use_fused=use_fused)
    x = torch.randn(B, T, E)
...
def test_attention_probs_are_rowwise_probabilities(use_fused, monkeypatch):
    ...
    attn = CausalSelfAttention(E, H, T, dropout_p=0.0, use_fused=use_fused)
...
def test_cuda_parity_with_cpu():
    ...
    cpu = CausalSelfAttention(E, H, T, dropout_p=0.0, use_fused=True).eval()
