...
def get_rope_angles(pos_ids: Tensor, inv_freq: Tensor):
    ...
    # interleave along the last dim: (cos0, cos0, cos1, cos1, ...)
    sin = torch.stack((sin, sin), dim=-1).view(*sin.shape[:-1], sin.shape[-1] * 2)
    cos = torch.stack((cos, cos), dim=-1).view(*cos.shape[:-1], cos.shape[-1] * 2)
    return cos, sin
...
def make_pos_ids(batch_sizes: Tensor, past_sizes: Tensor):
    ...
    pos = ids.unsqueeze(0) + past_sizes.unsqueeze(0)
    return pos[:, :T]
